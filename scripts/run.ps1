...
# Same single canonical boot path as Start-DeepSeek-Harness.cmd and every
# shortcut: electron.exe <root>\app\desktop-main.cjs with app\ as cwd.
$entry = "$ROOT\app\desktop-main.cjs"
if (-not (Test-Path -LiteralPath $electron)) { throw 'Electron not installed. Run install.ps1 first.' }
if (-not (Test-Path -LiteralPath $entry)) { throw "Harness entry point missing: $entry" }
$logDir = "$ROOT\logs\app"
New-Item -ItemType Directory -Path $logDir -Force | Out-Null
$proc = Start-Process -FilePath $electron -ArgumentList @("`"$entry`"") -WorkingDirectory "$ROOT\app" -WindowStyle Hidden -RedirectStandardOutput "$logDir\electron.out.log" -RedirectStandardError "$logDir\electron.err.log" -PassThru
