...
/** Scratch copies never inherit the dependency tree or the developer's data. */
const SKIPPED_COPY_DIRS = new Set(['node_modules', 'data'])
...
/**
 * Copy the shipped app sources into a scratch root.
 *
 * `node_modules` is skipped on purpose: the scratch app receives a junction to
 * the real dependency tree below, and copying it would both be enormous and fail
 * on the links inside it (EPERM). Runtime `data` is skipped for the same reason
 * the unit tests use a fresh root: the scenario must not inherit the developer's
 * themes, skills or credentials.
 */
  ...
  if (mode === 'C' || mode === 'all') {
    ...
    await runShell({
      ...
      extraChecks: async ({ root: bootRoot }) => {
        ...
        const mergedFiles = ['e2e-alpha.txt', 'e2e-beta.txt']
        // The merge runs on the tick after the DAG drains, and the worktree
        // directory exists a moment before the merged files land in it. Waiting
        // for the files (not the directory) is the real synchronization point;
        // the assertions below still fail if they never arrive.
        await waitFor(
          () => mergedFiles.every((file) => fs.existsSync(path.join(integrationWorktree, file))),
          { label: 'the merged integration files', timeoutMs: 60_000 }
        )
        check('adaptive: an integration worktree was produced', fs.existsSync(integrationWorktree), integrationWorktree)
        const merged = mergedFiles.filter((file) => fs.existsSync(path.join(integrationWorktree, file)))
