...
test('the compatibility flow is wired end to end: store, shell dialog, bridge and panels', () => {
  ...
  assert.match(pluginPanel, /plugin\.compatibility === 'compat'[\s\S]{0,300}compat · /, 'the platform panel does not label an adopted plugin')
  assert.match(pluginPanel, /兼容模式不保证 · not guaranteed/, 'the platform panel never states the reduced guarantees')
