...
test('the Harness launch path is untouched', () => {
  ...
  assert.match(main, /const HARNESS_PORT = .*: 3080/, 'the default Harness port stays 3080')
  ...
  // The additive port override exists only for isolated regression runs and
  // never adds a launch argument unless it is explicitly set.
  assert.match(main, /if \(Number\(process\.env\.DSH_HARNESS_PORT\) > 0\) harnessArgs\.push\('--port', String\(HARNESS_PORT\)\)/)
  const defaultArgsLine = main.slice(main.indexOf('const harnessArgs ='), main.indexOf('harnessProcess = spawn('))
  assert.match(defaultArgsLine, /const harnessArgs = \[DSH_ENTRY, 'web', '--no-open'\]/)
