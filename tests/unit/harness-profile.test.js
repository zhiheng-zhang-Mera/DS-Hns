...
test('the launch refreshes the profile copy before the Harness is spawned', () => {
  ...
  assert.match(main, /modulesDir: path\.join\(process\.env\.DSH_HOME, 'profiles', profile, 'node_modules'\)/)
  assert.doesNotMatch(main, /modulesDir: path\.join\(ROOT, 'data', 'profiles', profile, 'node_modules'\)/)
