...
> 详细功能放同窗可切换的“监控/设置”页(不弹新窗口)。
> 铃声(ringtone)已做成 **用户可自定义 + 可选开关**。

## 特性

- **以 Alien 桌面壳为基准的单进程体验**:不占用系统浏览器;单进程锁防重复服务(再次启动默认**聚焦已有窗口**,`DSH_MULTI=1` 可多开窗口)、日志令牌脱敏、端口自动错开、关闭即停引擎。
- **左侧历史栏可收放**(`«/»`),收起后专注对话;状态记忆于本机。
...
2. 页面内导航(或 `Ctrl+1..3`)切换:主界面对话 / 监控 / 设置(同窗详细视图,不弹新窗口)。
3. 在 **设置 → 铃声** 中:
   - 勾选“启用铃声(总开关)”控制全部铃声;
   - 拖动“播放音量”;
...
5. **ChatGPT/Codex 式交互**:消息支持 **复制回答 / ■停止(运行中、排队、挂起)/ ↻再跑**;Composer 输入框随内容自动增高;`Enter` 发送、`Shift+Enter` 换行、`Esc` 关闭弹层/退出批量。
6. **项目工作区(设置页)**:可浏览/输入/打开目录并把任意目录设为任务工作区(headless 任务在 `工作区\active\<任务ID>` 执行,环境 `DEEPSEEK_HARNESS_WORKSPACE` 同步)。
7. **左侧历史管理**:右键某条历史可 打开 / 重命名(显示名存于本地元数据) / 移动到文件夹… / 删除 / 进入批量操作;
