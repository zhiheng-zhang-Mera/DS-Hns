...
- **监控/设置(同窗详细视图,不弹新窗口)**,端口 `http://127.0.0.1:3300`:
  ...
  - 菜单“视图”或 `Ctrl+1..3` 在主界面/监控/设置间切换。
- **官方 dsh Web 视图(可选)**:Alien 原版官方界面仍保留在 视图菜单 `Ctrl+4`,按需启动引擎加载;
...
2. 菜单 **视图** 或 `Ctrl+1..3` 切换:主界面对话 / 监控 / 设置(同窗详细视图,不弹新窗口);`Ctrl+4` = 官方 dsh Web 视图(按需启动引擎)。
3. 在 **设置 → 铃声** 中:
   - 勾选“启用铃声(总开关)”控制全部铃声;
   - 拖动“播放音量”;
