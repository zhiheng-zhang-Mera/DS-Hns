...
- Current remote PR #5 targets `main` and remains open. CI run `36120978009` passed at documentation/evidence head `f59f0f1d1aa84ec409d63eaaa9b07b56c0783068`, whose product source candidate remains `4ef9c0df149a6cecf1eaa17e50cee4b22271b810`. Current `main` SHA `8b91228628e9168cabd545f26f1320ba141561e0` and `hns-production-v1` tag object/peeled target (`2d4aeeda…` / `5dcde767…`) were not changed by this work.
...
The exact-candidate wall-clock observer began at `2026-09-25T06:59:04.9297139Z` under `D:\qf\realwallclock-4ef9c0d-20260925`, with a one-minute cadence. At the latest report snapshot (`2026-09-25T10:20:04.8974533Z`) it had 202 samples across 11,999.968 seconds (3.333324 hours); the Runtime Host, Harness, and Electron were attached and ready in every sample, with 0 status errors and 0 runtime-down, Electron-detached, or Harness-not-ready samples. There were 8 matching product processes at the latest sample; maximum sample gap was 60.024 seconds. The raw process count ranged from 8 to 12: samples 99–108 included the separate Electron default-app shell and three children from Computer Use recovery; samples 178–179 overlapped an unintended full combined run and samples 182–183 overlapped the three final-candidate Phase C repeats. The shell was visibly closed and test processes exited. Aggregate working set ranged from 733,396,992 to 1,392,148,480 bytes, private bytes from 665,014,272 to 1,077,383,168 bytes, and handles from 3,413 to 5,671; latest values were 739,897,344 bytes, 730,087,424 bytes, and 3,455. Peak values include transient test processes and the separate shell and are not attributable solely to DS-Hns. Two provider UI requests were not instrumented/correlated to samples. The observer remains partial: no long task, checkpoint-continuity sweep, plugin fault/recovery injection, reboot, or internal event-loop drift was measured. It is not 24-hour or full-soak acceptance.
...
- `STORAGE-C-COMBINED-ACCEPTANCE-TEMP-001` (open blocker): an unsupported `--help` argument fell through to the combined acceptance script's default A–E run while inherited `TEMP/TMP` pointed to `C:\Users\15601\AppData\Local\Temp`. The fixture used `os.tmpdir()` and created then removed its scratch during normal teardown; a later read-only check found no matching directories. The C-drive write event remains recorded (report SHA-256 `ce9856b380eec58846295b31fb31ab6d70dc19072bea7416ad010c66e802eba5`); the acceptance report is retained on D: and is not used as D-compliant qualification. Do not invoke that script with unsupported flags.
