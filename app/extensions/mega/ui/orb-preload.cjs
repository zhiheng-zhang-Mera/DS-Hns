 ...
 * reachable surface** of that window. It can ask for the current state, say what happened, and ask for one of
 * the governance actions — and nothing else. There is no channel that takes a path, a command or a function,
 * and no dynamic channel name, so the surface can be enumerated rather than guessed at.
 */
