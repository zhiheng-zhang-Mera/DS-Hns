...
function discoveryFile(env = process.env) {
  ...
  return path.join(home, 'state', 'governance-bridge.json')
}
...
export function readDiscovery(env = process.env) {
  ...
  try {
    ...
    return { available: true, host: parsed.host || '127.0.0.1', port: Number(parsed.port), token: String(parsed.token), pid: parsed.pid || null, file }
  } catch (error) {
  ...
  disposers.push(webServer.register({
    ...
    handler: async (_req, res) => {
      ...
      answer(res, 200, {
        ...
        governance: discovery.available ? { available: true, host: discovery.host, port: discovery.port, pid: discovery.pid } : { available: false, reason: discovery.reason }
      })
    }
  }))
