...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * The conversation header's entry point.
		 *
		 * It sits with the official header actions (the schedule clock, the jobs list) rather than inside the
		 * composer: it is a place you *go* to start something, and the composer is where you are already typing.
		 */
		function NewTaskAction() {
			const [open, setOpen] = React.useState(false);
			if (!primitives) return null;
			return box('span', { 'data-hns-mega-new-task-action': 'on', style: { display: 'inline-flex' } }, [
				React.createElement(primitives.Button, {
					key: 'trigger',
					variant: 'ghost',
					size: 'sm',
		...
		function apply(ctx) {
			...
			if (ctx && ctx.slots) {
				...
				/**
				 * The new-task entry, beside the official header actions. `order: 30` puts it after the jobs list
				 * and the schedule clock (both in the lower twenties) and before nothing else — the slot is a list,
				 * so the shipped entries are added beside, never displaced.
				 */
