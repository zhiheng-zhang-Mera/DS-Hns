...
Current inventory: 7 claim rows (6 bounded engineering claims, 1 explicitly unsupported comparator claim), 20 experiment/acceptance rows, 11 metrics, and 46 negative-result rows. It retains historical failed UI evidence, real reboot `NOT_RUN`, an exact-candidate wall-clock liveness/resource observer `IN_PROGRESS_PARTIAL` (not a full task/provider soak), the passing exact RC machine qualification, and a partial direct visual journey with one live provider smoke as distinct states. Counts describe catalog entries, not independent samples or research contributions. Null experiment fields mean unrecorded or unexecuted; consult the hash-bound evidence before using a value as a measurement. A passing child from an overall failed historical run is not final qualification. Fresh17 follow-up repairs extend engineering lineage, not the research-claim count; see `artifacts/qualification/FRESH17_VISUAL_FOLLOWUP.md` at repository root.

History cutoffs are explicit: the production-baseline snapshot covers 366 reachable commits; after fetching all remotes/tags, the later fixed full-history snapshot at `d4d2e8757c7660a6130050cc718debf79c1ea252` covers 379. The 13-commit delta contains 2 merges and 11 non-merge diffs with 11 distinct stable patch-ids; the timeline records each commit's reviewed change. This is not a contribution count.
