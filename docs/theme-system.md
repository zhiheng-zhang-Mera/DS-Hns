...
- **底盘不着色**：底板（`--hns-color-bg-base`）在这一层打开时是 `transparent`，即底盘只有模糊、
  没有颜色——Dock 盖在官方界面上，任何一点着色都是从别人的界面里拿走的颜色，
  也会与被覆盖的控件串色。模块靠自身极淡的着色与 1px 边框划出边界，这是能看出边界的最小用量。
- **下限 5%**：近乎全透明。这个下限是给「模糊自己就能说明问题」的场合用的；0 不是「更透明」
  而是「关掉」，而关掉是开关的事。
