...
**端到端验收脚本**：`node scripts/sub-worker-acceptance.cjs [A|B|C|all]`（不传参数等同于 `all`）。它**不属于单元套件**：它启动真实的 Electron shell 三次，因此必须完全隔离，绝不打扰已经在运行的 DS-Harness —— 脚本先把 `app/` 复制到 `temp/e2e-default` / `temp/e2e-enabled` / `temp/e2e-adaptive` 的临时 root（`app/node_modules` 用 `mklink /J` 做成 junction，不重复下载依赖），再往该 root 写一份自己的 `config/app.json`（把 `harness.port` 从 3080 挪开：A 用 3210、B 用 3211、C 用 3212）并拷贝 `config/.env`，最后用 `DSH_ROOT` / `DSH_HOME` / `DSH_HARNESS_PORT` / `DSH_STARTUP_TIMEOUT_MS` / `DSH_MEGA_OBSERVE_MS` 指向该 root 启动 `electron app`；临时副本保留在 `temp/e2e-*` 供事后检查。
...
| **C（自适应多进程）** | `data/sub-worker/config.json` = `{"enabledOnStartup": true, "adaptiveWorkers": true, "resources": {...hardMax: 2, 1 秒伸缩延迟...}}`，并预置一份 `state.json`，其中 `active_plans` 是一个**两节点**计划（节点任务只写 `operations`，故意不重复 `target_repo`，以验证恢复时的再水化） | 共享断言（此处断 managers「就绪」而非「惰性」）；**额外**：`data/sub-worker/hardware-profile.json` 由真实探测生成（CPU/RAM/存储齐备且上限 ≥ 1）；池增长到**恰好 2 个真实 worker 进程**且不超过配置上限；`logs/sub-worker/supervisor.log` 记录 `adaptive true`；`resources.log` 有采样；两个恢复节点在真实 shell 上**都完成**；产生 `hns-e2e-adaptive-integration` 集成 worktree 且其中同时含两个节点的产物；目标仓库自身**未被写入**；`metrics.json` 有 ≥ 2 条记录 |
...
| **退出之后（B/C）** | 同上 | 没有孤儿 worker 进程；`runtime/sub-worker-process.json` 已清除；`data/sub-worker/state.json` 的 `state === "OFF"`（C 还额外断言 N=2 时同样无孤儿） |
